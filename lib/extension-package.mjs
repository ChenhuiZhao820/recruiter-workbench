...
export function extensionFiles(source, originValue, { allowLoopback = false } = {}) {
  ...
  // The one optional origin is LinkedIn, and it is exactly that: not granted at
  // install, asked for by the recruiter in the panel, revocable by them. Any
  // other optional permission, or any registered script, is not ours.
  if (manifest.background || manifest.content_scripts || manifest.optional_permissions ||
      JSON.stringify(manifest.optional_host_permissions) !== JSON.stringify(["https://*.linkedin.com/*"]) ||
      JSON.stringify(manifest.permissions) !== JSON.stringify(["activeTab", "scripting", "sidePanel", "storage"])) {
