...
test.beforeAll(async () => {
  ...
  for (const [fullName, stage, profileUrl] of people) {
    const candidate = await db.candidate.create({
      data: { roleId: role.id, fullName, stage, profileUrl: profileUrl || null },
    });
...
test("Q3 one person at a time, with the message already written for them", async ({ page }) => {
  ...
  await expect(page.getByText("Hi Quinn,")).toBeVisible();
  await expect(page.getByText("I'm hiring for a Queue Test Director.")).toBeVisible();
  ...
  await expect(page.getByRole("link", { name: "Open profile" })).toHaveAttribute(
    "href",
    "https://www.linkedin.com/in/quinn-sourced"
  );
...
test("Q9 an administrator viewing somebody else's workspace can look but not record", async ({ page }) => {
  ...
  try {
    ...
    // Without an href it is not a link at all, only the label of one.
    await expect(page.getByRole("link", { name: "Open profile" })).toHaveCount(0);
    await expect(page.locator('a[aria-disabled="true"]', { hasText: "Open profile" })).toBeVisible();
